...
public class UserCommandService(IUserRepository userRepository, IUnitOfWork unitOfWork, IHashingService hashingService) : IUserCommandService
{
    public async Task<User?> Handle(CreateUserCommand command)
    {
        if (await userRepository.ExistsByEmailAsync(command.Email)) return null;
        
        var user = new User(command.FullName, command.Email, hashingService.HashPassword(command.Password), command.Role);
